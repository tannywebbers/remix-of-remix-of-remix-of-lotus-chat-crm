...
serve(async (req) => {
  ...
  try {
    ...
    console.log('WhatsApp API action:', action, params);
    ...
    switch (action) {
      case 'test_connection': {
        ...
        if (!response.ok) {
          ...
          return new Response(
            JSON.stringify({ success: false, error: error.error?.message || 'Connection failed' }),
        ...
        return new Response(
          JSON.stringify({ success: true, phoneNumber: data.display_phone_number }),
          { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }
      ...
      case 'sync_templates': {
        ...
        if (!response.ok) {
          ...
          return new Response(
            JSON.stringify({ success: false, error: error.error?.message || 'Failed to fetch templates' }),
            { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
          );
        }
        ...
        if (type === 'template' && templateName) {
          ...
          messageBody.template = {
            ...
            components: templateParams ? [{
              type: 'body',
              parameters: Object.values(templateParams).map((v: any) => ({ type: 'text', text: v })),
            }] : [],
          };
      ...
      case 'get_media': {
        ...
        if (!response.ok) {
          return new Response(
            JSON.stringify({ success: false, error: 'Failed to get media' }),
            { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
          );
        }
